...
[![Tests](https://img.shields.io/badge/pytest-94%20passed-brightgreen.svg)](https://docs.pytest.org/)
...
|---|---|---|
| **[Спецификация архитектуры системы](docs/ARCHITECTURE_RU.md)** | Backend-разработчики, архитекторы | Архитектурные инварианты, топология AMQP, блокировки Redis на Lua, чанковая обработка, безопасность SSRF, компромиссы (trade-offs) |
| **[Руководство разработчика по интеграции](docs/DEVELOPER_GUIDE_RU.md)** | Backend-разработчики, инженеры интеграций | Реестр обработчиков, bulk-upsert в PostgreSQL, вызовы внешних API с троттлингом, стриминг прогресса |
...
1. **Нагрузка на оперативную память при больших объемах:** Классические воркеры дублируют массивы данных в памяти при срезах и итерациях. В движке реализована генераторная обработка чанками (`chunk_iterator`), исключающая создание промежуточных списков и снижающая паузы сборщика мусора Python GC при пакетной обработке
...
6. **Паразитный опрос статуса через Polling (Защищенные вебхуки):** Опциональный параметр `callback_url` позволяет отправлять HTTP push-уведомления об успешном завершении или ошибках в DLQ; исходящие вызовы защищены от SSRF фильтрацией IP-адресов и криптографической HMAC-SHA256 подписью (`X-Hub-Signature-256`)
