...
- Задачи надолго зависают в статусе `RUNNING`, превышая таймауты выполнения

### Как дежурный инженер создает отчет об инциденте (Incident Dossier)

Сотрудник поддержки может сформировать диагностический отчет для разработчиков тремя способами:

1. **Глобальная кнопка в шапке веб-консоли:**
   - Открыть `http://localhost:8000/dashboard`
   - Нажать красную кнопку **`🚨 Create Incident`** в верхней панели управления
   - Ввести полученный от клиента `Task UUID` и наблюдения оператора
   - Нажать **Generate Incident Dossier**, затем **📋 Copy Dossier to Clipboard**
   - Вставить сформированный Markdown-отчет в тикет Jira или инцидентный чат разработки
2. **Из таблицы Dead-Letter Queue:**
   - В нижней таблице DLQ нажать кнопку **`🚨 Escalate`** напротив проблемной задачи
   - Модальное окно автоматически заполнит UUID задачи и запросит причину сбоя из Redis
3. **Через Operations API:**
   ```bash
